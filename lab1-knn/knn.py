...
def non_param_reg(dataset, val_column, query, distStr, kernelStr, windowStr, window):
    n = len(dataset)
    params = []
    values = []
    for row in dataset:
        params.append(row[:val_column] + row[val_column+1:])
        values.append(row[val_column])
